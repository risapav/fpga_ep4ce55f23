...
module AsyncFifoGeneric #(
    ...
    parameter bit TWO_STAGE_SYNC = 1        // 1 = 2-step Gray synchronizácia, 0 = 1-step
)(
    input  logic rstn,
    input  logic wr_clk,
    input  logic wr_en,
...
module SdramController #(
    ...
    typedef enum logic [4:0] {
      ...
      READ_BURST, WRITE_BURST} state_t;
    ...
    CountdownTimer #(
      .COUNT_WIDTH($clog2(tRP+1)),.ASYNC_RESET(1),.DONE_REGISTERED(1)
      ) trp_timer_inst (
        .clk(clk),.rstn(rstn),
        .load(load_trp),.load_val(tRP),
        .done(trp_done)
    );
    CountdownTimer #(
      .COUNT_WIDTH($clog2(tRCD+1)),.ASYNC_RESET(1),.DONE_REGISTERED(1)
      ) trcd_timer_inst(
        .clk(clk),.rstn(rstn),
        .load(load_trcd),.load_val(tRCD),
        .done(trcd_done)
    );
    CountdownTimer #(
      .COUNT_WIDTH($clog2(tWR+1)),.ASYNC_RESET(1),.DONE_REGISTERED(1)
      ) twr_timer_inst (
        .clk(clk),.rstn(rstn),
        .load(load_twr),.load_val(tWR),
        .done(twr_done)
    );
    CountdownTimer #(
      .COUNT_WIDTH($clog2(tRFC+1)),.ASYNC_RESET(1),.DONE_REGISTERED(1)
      ) trfc_timer_inst(
        .clk(clk),.rstn(rstn),
        .load(load_trfc),.load_val(tRFC),
        .done(trfc_done)
    );
    CountdownTimer #(
      .COUNT_WIDTH($clog2(tMRD+1)),.ASYNC_RESET(1),.DONE_REGISTERED(1)
      ) trmrd_timer_inst(
        .clk(clk),.rstn(rstn),
        .load(load_trmrd),.load_val(tMRD),
        .done(trmrd_done)
    );
    CountdownTimer #(
      .COUNT_WIDTH($clog2(INIT_WAIT_CYCLES+1)),.ASYNC_RESET(1),.DONE_REGISTERED(1)
      ) init_timer_inst(
        .clk(clk),.rstn(rstn),
        .load(load_init),.load_val(INIT_WAIT_CYCLES),
        .done(init_done)
    );
    AsyncFifoGeneric #(
      .DATA_WIDTH(DATA_WIDTH),.ADDR_WIDTH(FIFO_ADDR_WIDTH),.ASYNC_RESET(1)
      ) write_fifo_inst(
        .rstn(rstn),
    ...
    always_comb begin
        ...
        for (int i=0; i<NUM_BANKS; i++)
            if (tras_timer[i] > 0)
              tras_timer_next[i] = tras_timer[i] - 1;

        if (cas_cnt > 0)
          cas_cnt_next = cas_cnt - 1;
        ...
        case (state_reg)
            ...
            READ_BURST: begin
              if (cas_cnt == 0) begin
                if (burst_cnt > 0) burst_cnt_next = burst_cnt - 1;
                if (burst_cnt == 1) state_next = IDLE;
              end
            end
            WRITE_BURST: begin
