// AxiStreamSdramWrapper.sv - Verzia 1.60 - Synthesis-Optimized DMA Scheduler
...
// 3. OPTIMALIZÁCIA (Burst Coalescing): Pridané debug porty na sledovanie počtu
//    aktívnych čítaní a zápisov, čo pomáha pri ladení a optimalizácii výkonu.
...
module AxiStreamSdramWrapper #(
    ...
    always_ff @(posedge clk) begin
        if (!rstn) begin
            ...
            if (buffer_state[write_ptr] == EMPTY && s_axis_tready && s_axis_tvalid)
            ...
            if (issue_aggressive_lookahead_cmd && cmd_fifo_ready)
    ...
    always_comb begin
        ...
        // Adaptive burst coalescing: vydaj viac burstov, ak FIFO je voľné
