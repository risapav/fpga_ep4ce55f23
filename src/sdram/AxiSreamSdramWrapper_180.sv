// AxiStreamSdramWrapper.sv - Verzia 1.80 - Production-Hardened
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v1.80):
// 1. ROBUSTNOSŤ (Timeout): Implementovaný "watchdog" časovač, ktorý deteguje
//    zaseknutý AXI stream, vygeneruje chybu a bezpečne zresetuje modul.
// 2. VERIFIKÁCIA (Asercie): Pridané `assert` príkazy pre aktívnu kontrolu
//    nelegálnych stavov počas simulácie, čo urýchľuje ladenie.
// 3. FLEXIBILITA (Parameterizácia): Prahová hodnota pre odosielanie príkazov
//    do FIFO je teraz konfigurovateľná cez parameter `CMD_FIFO_THRESHOLD`.
//
// Zmeny (v1.70):
// 1. Implementovaná plná podpora pre variabilnú dĺžku rámcov pomocou `s_axis_tlast`.
// 2. Upravená logika `write_ptr` pre vyššiu robustnosť.
// 3. Zabezpečená kompatibilita so syntetizátorom Quartus Prime.
...
module AxiStreamSdramWrapper #(
    ...
    // REFAKTORING (v1.80): Nové parametre pre robustnosť a ladenie
    parameter integer STREAM_TIMEOUT_CYCLES = 1_000_000, // Počet cyklov nečinnosti pred vyhlásením chyby
    parameter integer CMD_FIFO_THRESHOLD = 4,      // Voľné miesto v CMD FIFO pred odoslaním príkazu
    ...
    output logic                       stream_timeout_error, // REFAKTORING (v1.80)
    ...
    logic internal_reset; // REFAKTORING (v1.80): Pre timeout
    ...
    always_ff @(posedge clk) begin
        // Kombinovaný reset (externý alebo interný z timeoutu)
        if (!rstn || internal_reset) begin
            ...
            if (s_axis_tuser_sof && word_accepted) begin
                ...
                if (word_accepted) begin
                    timeout_counter <= STREAM_TIMEOUT_CYCLES - 1; // Reset časovača
                end else begin
                    timeout_counter <= timeout_counter - 1;
                end
                if (s_axis_tlast && word_accepted) begin
                    timeout_active <= 1'b0; // Deaktivácia po úspešnom konci rámca
                end
            end
            ...
            if (word_accepted) begin
                // REFAKTORING (v1.80): Asercia pre verifikáciu
                `ifndef SYNTHESIS
    ...
    always_comb begin
        ...
        s_axis_tready = (buffer_state[write_ptr] == EMPTY || buffer_state[write_ptr] == FILLING) && (w_pipeline_level < PIPELINE_BUFFER_SIZE) && !internal_reset;

        // --- Logika Plánovača Príkazov (Command Scheduler) ---
        // REFAKTORING (v1.80): Použitie parametra pre prahovú hodnotu
        ...
        if (can_push_to_cmd_fifo && !internal_reset) begin
        ...
        wdata_valid = (w_pipeline_level > 0) && !internal_reset;
        wdata       = w_pipeline_buffer[w_pipeline_rptr];
        wdata_dqm_i = '0;

        m_axis_tvalid = resp_valid && (buffer_state[read_ptr] == READING) && !internal_reset;
