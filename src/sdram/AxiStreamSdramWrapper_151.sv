// AxiStreamSdramWrapper.sv - Verzia 1.51 - Robustná Double Buffering Architektúra
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v1.51):
// 1. VYLEPŠENIE (Robustnosť): Pridaná detekcia chýb v AXI-Stream pakete. Modul
//    teraz overuje, či `tlast` signál prichádza presne s posledným slovom paketu.
//    V prípade chyby sa nastaví výstup `error_o` a modul sa bezpečne zastaví.
// 2. VYLEPŠENIE (Robustnosť): Reset bol zmenený z asynchrónneho na plne synchrónny,
//    čo je preferovaná prax pre moderné FPGA návrhy a zjednodušuje timing closure.
...
module AxiStreamSdramWrapper #(
    parameter integer NUM_BUFFERS      = 2,
    ...
    // --- Výstupy ---
    output logic                     error_o, // VYLEPŠENIE (v1.6): Signalizácia chyby
    ...
    // --- Riadiace a chybové signály ---
    logic issue_write_cmd, issue_read_cmd;
    logic error_reg; // VYLEPŠENIE (v1.6): Register pre zachytenie chyby

    // --- Sekvenčná logika ---
    // VYLEPŠENIE (v1.6): Zmenené na plne synchrónny reset
    always_ff @(posedge clk) begin
        if (!rstn) begin
            ...
            // Logika beží len ak nenastala chyba
            if (!error_reg) begin
                ...
                if (!write_frame_done) begin
                    ...
                    end else if (issue_write_cmd && cmd_fifo_ready) begin // buffer sa resetuje až po úspešnom odoslaní príkazu
                        w_buffer_fill_count <= 0;
                    end
            ...
            // --- Logika pre detekciu a zachytenie chyby ---
            logic packet_error;
            packet_error = 1'b0;
            if(s_axis_tready && s_axis_tvalid) begin
                // Chyba 1: `tlast` prišiel skôr
                if (s_axis_tlast && (write_words_count < PACKET_LEN_WORDS - 1)) packet_error = 1'b1;
                // Chyba 2: `tlast` neprišiel s posledným slovom
                if (!s_axis_tlast && (write_words_count == PACKET_LEN_WORDS - 1)) packet_error = 1'b1;
            end
    ...
    always_comb begin
        ...
        // Modul prijíma dáta, len ak nie je hotový frame a nenastala chyba
        s_axis_tready = !write_frame_done && !error_reg;

        can_issue_write = (w_buffer_fill_count == BURST_LEN) && !write_frame_done && !error_reg;
        can_issue_read = buffer_ready_to_read[read_buf_sel] && !read_frame_done && (read_words_sent_count < PACKET_LEN_WORDS) && !error_reg;
        ...
        if (cmd_fifo_ready) begin
            if (can_issue_read) begin
                issue_read_cmd = 1'b1;
                cmd_fifo_valid = 1'b1;
                cmd_fifo_data  = '{addr: current_read_base + read_addr_offset, rw: READ_CMD, auto_precharge: 1'b0};
            end else if (can_issue_write) begin
                issue_write_cmd = 1'b1;
                cmd_fifo_valid = 1'b1;
                cmd_fifo_data   = '{addr: current_write_base + write_addr_offset, rw: WRITE_CMD, auto_precharge: 1'b0};
            end
        end
        ...
        m_axis_tvalid = resp_valid && !error_reg; // Zastaví výstup pri chybe
        m_axis_tdata  = resp_data;
        m_axis_tlast  = m_axis_tvalid && (read_words_sent_count == PACKET_LEN_WORDS - 1);
        resp_ready    = m_axis_tready || error_reg; // Zahodí zvyšné dáta pri chybe
    end
