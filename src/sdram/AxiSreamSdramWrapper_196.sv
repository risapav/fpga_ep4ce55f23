// AxiStreamSdramWrapper.sv - Verzia 1.96 - Quartus Genvar Compatibility Fix
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v1.96):
// 1. KOMPATIBILITA (Quartus): Deklarácia `genvar` bola presunutá mimo `for`
//    cyklu na úroveň modulu, aby sa zabezpečila kompatibilita so staršími
//    požiadavkami syntetizátora Quartus na syntax `generate` blokov.
//
// Zmeny (v1.95):
// 1. Funkcia `gen_buffer_addrs` bola nahradená za `generate` blok.
...
module AxiStreamSdramWrapper #(
    ...
    // REFAKTORING (v1.96): Deklarácia `genvar` mimo `for` cyklu pre maximálnu kompatibilitu.
    genvar i;
