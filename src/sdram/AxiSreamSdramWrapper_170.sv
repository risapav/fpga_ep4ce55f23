// AxiStreamSdramWrapper.sv - Verzia 1.70 - Refactored for Variable Frame Length
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v1.70):
// 1. ROBUSTNOSŤ (Variabilná Dĺžka): Implementovaná plná podpora pre variabilnú
//    dĺžku rámcov pomocou `s_axis_tlast`. Modul už nie je závislý od
//    pevného parametra `FRAME_LEN_WORDS`.
// 2. ARCHITEKTÚRA (Write Pointer): Logika pre inkrementáciu `write_ptr` bola upravená.
//    Ukazovateľ sa posunie až po vydaní posledného príkazu na zápis pre daný
//    segment, čo zvyšuje robustnosť pri spracovaní `tlast`.
// 3. KOMPATIBILITA (Quartus): Všetky lokálne deklarácie v `always_comb` bloku
//    boli presunuté na jeho začiatok pre bezproblémovú syntézu v Quartus Prime.
//
// Zmeny (v1.60):
// 1. OPTIMALIZÁCIA (Adresovanie): Násobenie pri výpočte adries bufferov bolo nahradené
//    statickým poľom predpočítaných adries (`BUFFER_BASE_ADDRS`).
// 2. ROBUSTNOSŤ (tlast): Logika pre generovanie `m_axis_tlast` bola zjednodušená.
// 3. OPTIMALIZÁCIA (Burst Coalescing): Pridané debug porty.
...
module AxiStreamSdramWrapper #(
    ...
    // REFAKTORING (v1.70): Ukladá presnú dĺžku každého segmentu
    ...
    always_ff @(posedge clk) begin
        if (!rstn) begin
            ...
            // REFAKTORING (v1.70): write_ptr sa posunie až po odoslaní posledného príkazu
            ...
            if (buffer_state[write_ptr] == FULL && is_last_write_burst) begin
                ...
                write_segment_word_count <= '0; // Reset pre nový segment
            end
            ...
            // Zápis je aktívny, ak je buffer v stave FILLING alebo FULL (čaká na zápis)
            dbg_active_writes <= $countones({buffer_state[3]==FILLING || buffer_state[3]==FULL,
                                             buffer_state[2]==FILLING || buffer_state[2]==FULL,
                                             buffer_state[1]==FILLING || buffer_state[1]==FULL,
                                             buffer_state[0]==FILLING || buffer_state[0]==FULL});
        end
    end

    // --- Kombinačná Logika ---
    always_comb begin
        // REFAKTORING (v1.70): Deklarácie na začiatku bloku pre kompatibilitu s Quartus
        ...
        can_push_to_cmd_fifo = (cmd_fifo_level_i < CMD_FIFO_DEPTH - 4);
        ...
        // --- Podmienky pre Čítanie (s prioritou Lookahead) ---
        ...
        // REFAKTORING (v1.70): tlast sa generuje na základe skutočnej dĺžky
