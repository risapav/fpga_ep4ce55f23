// sdram_controller_final.sv - Verzia 10.3 - Scope Fix for Outputs
//
// Zmeny (v10.3) - Oprava chyby rozsahu platnosti:
// 1. ROBUSTNOSŤ (Synthesis): Logika pre výstupy `resp_valid`, `resp_last`,
//    `resp_data` a `wdata_ready` bola presunutá z externých `assign`
//    priradení priamo do `always_comb` bloku. Tým sa opravila chyba
//    "object is not declared", ktorá vznikla, pretože tieto výstupy
//    záviseli od lokálnych premenných v `always_comb`.
//
// Zmeny (v10.2):
// 1. Opravená kritická chyba v stave `EVAL_CMD`, ktorá spôsobovala
//    nekonečnú slučku.
...
module SdramControllerFinal #(
    ...
    always_comb begin
        ...
        // OPRAVA (v10.3): Výstupné priradenia sú teraz vnútri `always_comb`,
        // pretože závisia od lokálnych premenných ako `fifo_r_empty`.
