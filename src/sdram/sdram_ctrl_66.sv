// sdram_controller.sv - Verzia 6.6 - Refaktorovaná na základe technickej analýzy
// Zmeny (v6.6):
// 1. PRIDANÉ (tRAS): Implementovaná explicitná kontrola časovania tRAS (Activate to Precharge)
//    pomocou nového časovača `tras_timer`. Tým sa zabráni predčasnému príkazu PRECHARGE.
// 2. KONTROLA (DQ Reset): Overené, že `dq_write_enable_d` je správne resetovaný. Analýza
//    naznačovala možný problém, ale kód bol v tomto bode už korektný.
// 3. ZMENA (IDLE Guard): Ochranná podmienka v stave IDLE bola rozšírená o kontrolu `tras_timer`.
...
module SdramController #(
    ...
    parameter int tRAS         = 7, // Parameter pre tRAS je teraz aktívne využívaný
    ...
    logic [$clog2(tRAS+1)-1:0]               tras_timer; // PRIDANÉ (v6.6): Časovač pre tRAS
    ...
    logic decrement_burst, last_read_beat, refresh_pending_next;
    logic auto_precharge_pending_next;
    logic [1:0] auto_precharge_bank_next;
    logic dq_keep_drive, pipe_can_shift;
    ...
    always_ff @(posedge clk or negedge rstn) begin
        if (!rstn) begin
            ...
            dq_write_enable_d      <= 1'b0; // KONTROLA (v6.6): Toto bolo správne aj v predchádzajúcej verzii
            ...
            if (load_tras) tras_timer <= tRAS; else if (tras_timer > 0) tras_timer <= tras_timer - 1; // PRIDANÉ (v6.6)
    ...
    always_comb begin
        ...
        case (state_reg)
            ...
            IDLE: begin
                // ZMENA (v6.6): Pridaná kontrola `tras_timer` do ochrannej podmienky.
            ...
            ACTIVE_CMD: begin
                ...
                load_trcd = 1'b1;
                load_tras = 1'b1; // PRIDANÉ (v6.6): Spustenie tRAS časovača pri aktivácii riadku
            ...
            READ_BURST: begin
                ...
                if (cas_cnt == 0 && resp_ready) decrement_burst = 1'b1;
    ...
    always_ff @(posedge clk) begin
        if (rstn) begin
            ...
            if ((state_reg == PRECHARGE_CMD) && (tras_timer > 0)) // PRIDANÉ (v6.6)
