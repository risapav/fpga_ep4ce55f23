// AxiStreamSdramWrapper.sv - Verzia 1.93 - Final Quartus Compatibility Fix
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v1.93):
// 1. KOMPATIBILITA (Quartus): Logika pre dočasné premenné bola presunutá
//    z bloku `always_ff` do `always_comb`, čím sa odstránila syntaktická
//    nepodpora lokálnych premenných v `always_ff` v niektorých verziách Quartusu.
//
// Zmeny (v1.92):
// 1. Deklarácie lokálnych premenných v `always_ff` boli presunuté na jeho začiatok.
//
// Zmeny (v1.91):
// 1. Nahradenie operátora `inside` pre lepšiu kompatibilitu.
...
module AxiStreamSdramWrapper #(
    ...
    // REFAKTORING (v1.93): Kombinačné signály pre next-state logiku
    ...
    always_comb begin
        ...
        // REFAKTORING (v1.93): Výpočet dočasných signálov v kombinačnom bloku
