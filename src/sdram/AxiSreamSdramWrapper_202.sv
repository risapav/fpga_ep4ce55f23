// AxiStreamSdramWrapper.sv - Verzia 2.02 - Synthesis-Hardened Reset
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v2.02):
// 1. ROBUSTNOSŤ (Synthesis): Procedurálny `for` cyklus v resetovacej logike
//    bol nahradený za robustné agregačné priradenie (`'{default:...}`).
//    Tento prístup je pre syntetizátor jednoznačnejší a predchádza
//    problémom s kompatibilitou v nástrojoch ako Quartus.
//
// Zmeny (v2.01):
// 1. Aritmetika pre inkrementáciu ukazovateľov bola upravená tak, aby používala
//    modulo operátor pre zaručenie správnej funkčnosti pre akýkoľvek počet bufferov.
...
module AxiStreamSdramWrapper #(
    ...
    // --- Lokálne parametre a generovanie adries ---
    ...
    // Kombinačné signály pre výpočet nasledujúceho stavu
    ...
    // --- Sekvenčná logika (Stavové registre) ---
    always_ff @(posedge clk) begin
        // Asynchrónny reset a synchrónne resety (softvérový a z timeoutu)
        if (!rstn || internal_reset || soft_reset_i) begin
            // REFAKTORING (v2.02): Nahradenie procedurálneho `for` cyklu za robustné
            // agregačné priradenie pre reset polí. Tým sa predchádza problémom
            // so syntaxou v niektorých verziách syntetizátora Quartus.
            buffer_state <= '{default:EMPTY};
            read_cmd_issued <= '{default:1'b0};
            segment_actual_len_words <= '{default:'0};
            ...
            // Watchdog časovač pre detekciu zaseknutého streamu
            ...
            if (buffer_state[write_ptr] == FULL && is_last_write_burst_comb) begin
                write_ptr <= (write_ptr + 1) % NUM_BUFFERS;
                write_segment_burst_count <= '0;
                write_segment_word_count <= '0;
            end

            // --- Čítacia cesta (Read Path) ---
            ...
            // Registrácia debugovacích hodnôt
    ...
    always_comb begin
        ...
        // Riadenie resetu a spätného tlaku (backpressure)
        stream_timeout_error = timeout_active && (timeout_counter == 0);
        internal_reset = stream_timeout_error;
        s_axis_tready = (buffer_state[write_ptr] == EMPTY || buffer_state[write_ptr] == FILLING) &&
                        (w_pipeline_level < PIPELINE_BUFFER_SIZE) &&
                        !internal_reset && !soft_reset_i;

        // Výpočet pomocných signálov
        word_accepted_comb = s_axis_tready && s_axis_tvalid;
        close_write_segment_comb = word_accepted_comb &&
                                   (s_axis_tlast || (write_segment_word_count == SEGMENT_LEN_WORDS - 1));
        ...
        last_word_of_read_segment_comb = m_axis_tvalid && m_axis_tready &&
        ...
        can_issue_write = ((w_pipeline_level >= PIPELINE_WRITE_THRESHOLD_WORDS) ||
                           (buffer_state[write_ptr] == FULL && w_pipeline_level > 0)) &&
        ...
        // Agresívne dopredné čítanie (Read Lookahead)
        can_issue_read_lookahead = 1'b0;
        lookahead_ptr = '0;
        for (int j = 0; j < NUM_BUFFERS; j = j + 1) begin
            logic [BUFFERS_ADDR_WIDTH-1:0] ptr = (read_ptr + j) % NUM_BUFFERS;
        ...
        // Prioritné vydávanie príkazov do SDRAM kontroléra
        ...
        // --- Dátová cesta (AxiStream -> SDRAM a SDRAM -> AxiStream) ---
        ...
        for (int j = 0; j < NUM_BUFFERS; j = j + 1) dbg_active_reads_comb += read_cmd_issued[j];

        dbg_active_writes_comb = '0;
        for (int j = 0; j < NUM_BUFFERS; j = j + 1) begin
             if (buffer_state[j] == FILLING || buffer_state[j] == FULL) dbg_active_writes_comb += 1;
        end
