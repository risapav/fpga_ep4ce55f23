// AxiStreamSdramWrapper.sv - Verzia 2.11 - Burst-Aware Write Logic
//
// Zmeny (v2.11) - Definitívna oprava zaseknutia:
// 1. ROBUSTNOSŤ (Burst-Aware): Logika pre vydávanie príkazu na zápis
//    (`can_issue_write`) bola upravená. Wrapper teraz pošle `WRITE_CMD` až
//    vtedy, keď má vo svojom pipeline bufferi nazbierané dáta pre aspoň
//    jeden celý burst (`WORDS_PER_BURST`). Tým sa zaručí, že SDRAM kontrolér
//    sa nikdy nezasekne v stave WRITE_BURST z dôvodu nedostatku dát.
//
// Zmeny (v2.10):
// 1. Opravená systematická syntaktická chyba pri nulovaní registrov.
...
module AxiStreamSdramWrapper #(
    ...
    always_ff @(posedge clk) begin
        if (!rstn || internal_reset || soft_reset_i) begin
            ...
            segment_total_bursts <= '{default:'0};
    ...
    always_comb begin
        ...
        logic [BUFFERS_ADDR_WIDTH-1:0] ptr;
        ...
        // OPRAVA (v2.11): Príkaz na zápis sa vydá, len ak máme dosť dát pre celý burst,
        // alebo ak zatvárame posledný (potenciálne neúplný) segment.
        can_issue_write = (w_pipeline_level >= WORDS_PER_BURST || (buffer_state[write_ptr] == FULL && w_pipeline_level > 0)) &&
