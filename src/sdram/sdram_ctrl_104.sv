// sdram_controller_final.sv - Verzia 10.4 - Explicit FSM Wait State Fix
//
// Zmeny (v10.4) - Oprava zaseknutia FSM:
// 1. ROBUSTNOSŤ (FSM): Logika v stave `EVAL_CMD` bola upravená tak, aby
//    explicitne zotrvala v tomto stave (`else state_next = EVAL_CMD;`),
//    pokiaľ sa čaká na časovače. Tým sa eliminuje akákoľvek nejednoznačnosť
//    pre syntetizátor a rieši sa problém s nekonečnou slučkou.
//
// Zmeny (v10.3):
// 1. Logika pre výstupy `resp_valid`, `resp_last`, `resp_data` a `wdata_ready`
//    bola presunutá priamo do `always_comb` bloku.
...
module SdramControllerFinal #(
    ...
    always_comb begin
        ...
        case (state_reg)
            ...
            EVAL_CMD: begin
                // OPRAVA (v10.4): FSM teraz explicitne čaká v stave EVAL_CMD,
                // kým sa časovače nevynulujú, aby sa predišlo slučeniu.
                if (bank_state[cmd_addr.bank] == BANK_IDLE) begin
                    if (trp_timer == 0) state_next = ACTIVATE_CMD;
                    else state_next = EVAL_CMD; // Čakaj tu
        ...
        // Výstupné priradenia sú teraz vnútri `always_comb`
