// sdram_controller.sv - Verzia 6.7 - Refaktoring s Read FIFO
// Zmeny (v6.7):
// 1. REFAKTORING (Read Pipeline -> Read FIFO): Pôvodný posuvný register pre čítané dáta
//    bol nahradený plnohodnotným synchrónnym FIFO bufferom s hĺbkou 16.
//    - Zvyšuje robustnosť: Úplne eliminuje riziko straty dát, ak je odberateľ dočasne
//      pozastavený (resp_ready = 0).
//    - Zlepšuje Backpressure: Controller teraz pozastaví čítanie z SDRAM, ak je
//      interný FIFO buffer plný.
// 2. ODSTRÁNENÉ: Signály `read_pipe_*` a `pipe_can_shift` boli nahradené logikou FIFO.
// 3. PRIDANÉ: Nové registre pre implementáciu FIFO (`read_fifo_data`, `read_fifo_last`,
//    `fifo_wptr`, `fifo_rptr`, `fifo_count`) a riadiace signály (`fifo_wr_en`, `fifo_rd_en`).
...
module SdramController #(
    ...
    // --- FIFO parametre a registre (PRIDANÉ v6.7) ---
    ...
    logic [$clog2(CAS_LATENCY+1)-1:0]        cas_cnt; // Šírka upravená pre presnosť
    ...
    always_comb begin
        ...
        // FIFO riadiace signály (PRIDANÉ v6.7)
        ...
        case (state_reg)
            ...
            READ_BURST: begin
                ...
                // ZMENA (v6.7): Burst sa dekrementuje len vtedy, ak je v cieli miesto (FIFO nie je plné).
    ...
    // ZMENA (v6.7): Výstupy sú teraz riadené z FIFO buffera.
