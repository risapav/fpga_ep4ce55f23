// AxiStreamSdramWrapper.sv - Verzia 1.91 - Quartus Compatibility Fix
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v1.91):
// 1. KOMPATIBILITA (Quartus): Operátor `inside` v logike pre `dbg_active_writes`
//    bol nahradený za ekvivalentnú logiku s `==` a `||` pre zaručenie
//    kompatibility so všetkými verziami syntetizátora Quartus.
//
// Zmeny (v1.90):
...
//
// Zmeny (v1.80):
// 1. Implementovaný "watchdog" časovač.
// 2. Pridané `assert` príkazy.
// 3. Parameterizovaná prahová hodnota pre FIFO.
