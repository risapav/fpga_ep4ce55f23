// AxiStreamSdramWrapper.sv - Verzia 1.95 - Synthesis Fix for Array Init
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v1.95):
// 1. KOMPATIBILITA (Quartus): Problematická funkcia `gen_buffer_addrs` pre
//    inicializáciu `localparam` poľa bola nahradená za robustný `generate`
//    blok. Tým sa odstraňuje chyba pri syntéze "assignments to unpacked
//    arrays must be aggregate expressions".
//
// Zmeny (v1.94):
// 1. Direktíva `ifndef SYNTHESIS` bola nahradená za `ifndef QUARTUS_SYNTH`.
...
module AxiStreamSdramWrapper #(
    ...
    // REFAKTORING (v1.95): Nahradenie funkcie za robustný generate blok pre inicializáciu adries.
    // Týmto sa predchádza chybám pri syntéze v Quartuse.
    logic [SDRAM_ADDR_WIDTH-1:0] BUFFER_BASE_ADDRS [NUM_BUFFERS-1:0];
    generate
        for (genvar i = 0; i < NUM_BUFFERS; i++) begin : gen_addr_calculator
            // Vypočítame adresu ako konštantu v rámci generate bloku
            localparam logic [SDRAM_ADDR_WIDTH-1:0] addr = SDRAM_BASE_ADDR + (i * SEGMENT_LEN_WORDS);
            // Priradíme konštantnú hodnotu signálu. Syntetizátor to optimalizuje.
