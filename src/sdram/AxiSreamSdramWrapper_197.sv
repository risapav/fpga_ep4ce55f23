// AxiStreamSdramWrapper.sv - Verzia 1.97 - Quartus $countones Synthesis Fix
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v1.97):
// 1. KOMPATIBILITA (Quartus): Nepodporovaná systémová funkcia `$countones` bola
//    nahradená za ekvivalentnú aritmetickú operáciu (sčítanie bitov),
//    aby sa zabezpečila bezproblémová syntéza.
//
// Zmeny (v1.96):
// 1. Deklarácia `genvar` bola presunutá mimo `for` cyklu.

`ifndef AXISTREAM_SDRAM_WRAPPER_SV
`define AXISTREAM_SDRAM_WRAPPER_SV

`define QUARTUS_SYNTH
...
module AxiStreamSdramWrapper #(
    ...
    always_ff @(posedge clk) begin
        if (!rstn || internal_reset || soft_reset_i) begin
            ...
            // --- Debug counters ---
            // REFAKTORING (v1.97): Nahradenie `$countones` za sčítanie pre syntézu
            dbg_active_reads <= read_cmd_issued[0] + read_cmd_issued[1] + read_cmd_issued[2] + read_cmd_issued[3];
            dbg_active_writes <= (buffer_state[3] == FILLING || buffer_state[3] == FULL) +
