// AxiStreamSdramWrapper.sv - Verzia 1.94 - Quartus `ifdef` Fix
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v1.94):
// 1. KOMPATIBILITA (Quartus): Direktíva `ifndef SYNTHESIS` bola nahradená za
//    `ifndef QUARTUS_SYNTH`, čo je správne makro, ktoré Quartus definuje
//    počas syntézy a zabezpečuje korektné vylúčenie simulačného kódu.
//
// Zmeny (v1.93):
// 1. Logika pre dočasné premenné bola presunutá z `always_ff` do `always_comb`.
...
module AxiStreamSdramWrapper #(
    ...
    always_ff @(posedge clk) begin
        ...
        if (!rstn || internal_reset || soft_reset_i) begin
            ...
            if (word_accepted_comb) begin
                // REFAKTORING (v1.94): Použitie správneho makra pre Quartus
