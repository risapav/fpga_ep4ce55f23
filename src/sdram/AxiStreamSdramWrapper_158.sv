// AxiStreamSdramWrapper.sv - Verzia 1.58 - Adaptive DMA Scheduler with Deep Command Pipelining
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v1.58):
// 1. ARCHITEKTÚRA (Deep Command Pipelining): Modul teraz aktívne spolupracuje s príkazovým
//    FIFO SDRAM kontroléra. Pomocou nového vstupu `cmd_fifo_level_i` dokáže posielať
//    dávky príkazov dopredu, čím plne využíva schopnosť kontroléra plánovať operácie.
// 2. VÝKON (Adaptive Arbitration): Arbitráž bola vylepšená, aby proaktívne udržiavala
//    príkazové FIFO optimálne naplnené. Tým sa dosahuje dynamické "segmentovanie"
//    prenosov na základe aktuálneho vyťaženia SDRAM, čo maximalizuje priepustnosť.
// 3. ROBUSTNOSŤ: Nahradenie závislosti od 1-bitového `cmd_fifo_ready` na viacbitový
//    `cmd_fifo_level_i` robí komunikáciu medzi wrapperom a kontrolérom robustnejšou.
...
module AxiStreamSdramWrapper #(
    ...
    parameter integer CMD_FIFO_DEPTH    = 16 // Hĺbka príkazového FIFO v SDRAM kontroléri
) (
    ...
    input  logic                     cmd_fifo_ready, // Stále môže byť použité pre spätnú kompatibilitu/jednoduchosť
    input  logic [$clog2(CMD_FIFO_DEPTH)-1:0] cmd_fifo_level_i, // VYLEPŠENIE: Aktuálne zaplnenie FIFO
    ...
    always_comb begin
        ...
        // VYLEPŠENIE: Podmienka na odoslanie príkazu je teraz viazaná na voľné miesto v cieľovom FIFO
        logic can_push_to_cmd_fifo = (cmd_fifo_level_i < CMD_FIFO_DEPTH - 1);
        ...
        if (can_push_to_cmd_fifo) begin
            if (issue_aggressive_lookahead_cmd) begin
                cmd_fifo_valid = 1'b1;
                cmd_fifo_data  = '{addr: SDRAM_BASE_ADDR + (lookahead_ptr * SEGMENT_LEN_WORDS), rw: READ_CMD, auto_precharge: 1'b0};
