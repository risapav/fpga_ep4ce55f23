// AxiStreamSdramWrapper.sv - Verzia 1.2 - Vylepšená robustnosť
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny:
// 1. OPRAVA: Odstránená chyba v logike počítadla `w_burst_sent_count`, ktoré sa
//    neresetovalo medzi jednotlivými burstami v rámci jedného paketu.
// 2. Vylepšená čitateľnosť a pridané komentáre k handshake logike.
...
module AxiStreamSdramWrapper #(
            ...
            case (state)
                ...
                WRITE_ISSUE_CMD: begin
                    if (cmd_fifo_ready) begin
                        ...
                        w_burst_sent_count <= 0;  // OPRAVA (v1.2): Reset počítadla odoslaných slov burstu
                    end
                end
                WRITE_SEND_BURST: begin
                    if (wdata_ready) begin
                        w_burst_sent_count <= w_burst_sent_count + 1;
                        // Inkrementácia adresy aj počas zápisu
    ...
    always_comb begin
        ...
        case (state)
            ...
            WRITE_ISSUE_CMD: begin
                // Držíme `cmd_fifo_valid` aktívny, kým kontrolér nie je pripravený (`cmd_fifo_ready`).
                // FSM zostane v tomto stave, kým handshake neprebehne.
                cmd_fifo_valid = 1'b1;
            ...
            WRITE_SEND_BURST: begin
                ...
                if (wdata_ready && (w_burst_sent_count == BURST_LEN - 1)) begin
                    // Použitie spoľahlivého flagu `write_packet_done`
