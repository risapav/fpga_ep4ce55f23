// sdram_controller.sv - Verzia 6.10 - Quartus Prime 24.1 Compatible
// Zmeny (v6.10):
// 1. ZMENA (Typová kompatibilita): Všetky deklarácie `parameter int` a `localparam int`
//    boli zmenené na `parameter integer` a `localparam integer` pre zaistenie
//    bezproblémovej kompilácie v Quartus Prime.
// 2. ZMENA (Port Compatibility): Výstupný port `debug_state_o` bol zmenený z typu
//    `state_t` na explicitný typ `logic [4:0]`, čo je robustnejší prístup
//    pre rôzne syntetizačné nástroje.
...
module SdramController #(
    ...
    parameter integer FIFO_DEPTH_BITS  = 4,      // ZMENA (v6.10): int -> integer
    parameter logic ENABLE_DEBUG = 1,
    parameter integer NUM_BANKS     = 2,         // ZMENA (v6.10): int -> integer
    parameter integer tRP             = 3,         // ZMENA (v6.10): int -> integer
    parameter integer tRCD            = 3,         // ZMENA (v6.10): int -> integer
    parameter integer tWR             = 2,         // ZMENA (v6.10): int -> integer
    parameter integer tRFC            = 9,         // ZMENA (v6.10): int -> integer
    parameter integer tRAS            = 7,         // ZMENA (v6.10): int -> integer
    parameter integer CAS_LATENCY     = 3          // ZMENA (v6.10): int -> integer
)(
    input  logic                     clk,
    input  logic                     clk_sh,
    input  logic                     rstn,
    ...
    output logic [4:0]                                    debug_state_o // ZMENA (v6.10): state_t -> logic [4:0]
);

    import sdram_pkg::*;

    localparam integer NS_PER_SEC         = 1_000_000_000; // ZMENA (v6.10): int -> integer
    localparam integer CLK_PERIOD_NS      = NS_PER_SEC / CLOCK_FREQ_HZ; // ZMENA (v6.10): int -> integer
    localparam integer WAIT_TIME_NS       = 200_000; // ZMENA (v6.10): int -> integer
    localparam integer INIT_WAIT_CYCLES   = WAIT_TIME_NS / CLK_PERIOD_NS; // ZMENA (v6.10): int -> integer

    localparam integer REFRESH_INTERVAL = (7812 * (CLOCK_FREQ_HZ / 1_000_000)) / 1000; // ZMENA (v6.10): int -> integer
    localparam integer C_COLS = 9; // ZMENA (v6.10): int -> integer
    ...
    localparam integer FIFO_DEPTH = 1 << FIFO_DEPTH_BITS; // ZMENA (v6.10): int -> integer
    localparam [$clog2(FIFO_DEPTH+1)-1:0] FIFO_DEPTH_CAST = FIFO_DEPTH;
    ...
    always_comb begin
        ...
        fifo_full  = (fifo_count == FIFO_DEPTH_CAST);
        ...
        case (state_reg)
            ...
            READ_BURST: begin
                ...
                if (last_read_beat && cas_cnt == 0 && !fifo_full) begin
                    ...
                    if (fifo_full) state_next = IDLE_WAIT;
    ...
    always_ff @(posedge clk) begin
        ...
        assert (!(fifo_wr_en && fifo_full)) else $error("[%0t] SVA ERROR: FIFO write on full!", $time);
        assert (!(fifo_rd_en && fifo_empty)) else $error("[%0t] SVA ERROR: FIFO read on empty!", $time);
        assert (fifo_count <= FIFO_DEPTH) else $error("[%0t] SVA ERROR: FIFO count overflow!", $time);
