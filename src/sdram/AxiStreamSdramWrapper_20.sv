// AxiStreamSdramWrapper.sv - Verzia 2.0 - Finálna verzia
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny:
// 1. VYLEPŠENIE VÝKONU: Predvolená dĺžka burstu (BURST_LEN) bola zvýšená na 16
//    pre efektívnejšie DMA prenosy a vyššiu priepustnosť.
// 2. PRIDANÉ (SVA): Doplnená kľúčová SystemVerilog Assertion, ktorá formálne
//    overuje, že zápis a čítanie nikdy nepristupujú k rovnakému bufferu súčasne.
// 3. PRIDANÉ (SVA): Doplnená nová SystemVerilog Assertion, ktorá kontroluje
//    integritu ukazovateľov round-robin schedulera.
...
module AxiStreamSdramWrapper #(
    ...
    parameter integer BURST_LEN         = 16, // ZMENA (v1.9): Zvýšené pre vyšší výkon
    parameter integer NUM_BUFFERS       = 4,
    parameter logic   ENABLE_DEBUG      = 1,
    ...
    generate
        if (ENABLE_DEBUG) begin : g_debug_outputs
            ...
            assign debug_find_empty_start_idx = find_empty_start_idx;
            assign debug_find_full_start_idx = find_full_start_idx;
        end else begin : g_no_debug_outputs
            for (genvar i = 0; i < NUM_BUFFERS; i++) begin
                assign debug_buffer_state[i] = EMPTY;
            end
            assign debug_find_empty_start_idx = '0;
            assign debug_find_full_start_idx = '0;
        end
    endgenerate

    // --- Formálna verifikácia (SVA) ---
    `ifndef SYNTHESIS
    always_ff @(posedge clk) begin
        if (rstn) begin
            // SVA: Kontrola, či buffer v stave READING prechádza len do stavu EMPTY
            if ($past(reading_active) && $rose(m_axis_tlast && m_axis_tready)) begin
                assert property (buffer_state[$past(read_buf_idx)] == EMPTY)
                else $error("[%0t] SVA Chyba: Buffer %0d v stave READING neprešiel do stavu EMPTY po dokončení čítania!", $time, $past(read_buf_idx));
            end

            // SVA: Kontrola, či buffer v stave FILLING prechádza len do stavu FULL
            if ($past(writing_active) && $rose(s_axis_tlast && s_axis_tready)) begin
                 assert property (buffer_state[$past(write_buf_idx)] == FULL)
                 else $error("[%0t] SVA Chyba: Buffer %0d v stave FILLING neprešiel do stavu FULL po dokončení zápisu!", $time, $past(write_buf_idx));
            end

            // SVA: Kontrola, či zápis a čítanie nikdy nepristupujú k rovnakému bufferu
            if (writing_active && reading_active) begin
                assert property (write_buf_idx != read_buf_idx)
                else $error("[%0t] SVA Chyba: Súčasný zápis a čítanie do rovnakého buffera (%0d)!", $time, write_buf_idx);
            end

            // PRIDANÉ (v2.0): SVA: Kontrola, či ukazovatele schedulera neprekročia rozsah
            assert property (find_empty_start_idx < NUM_BUFFERS)
                else $error("[%0t] SVA Chyba: Ukazovateľ find_empty_start_idx (%0d) prekročil rozsah!", $time, find_empty_start_idx);
            
            assert property (find_full_start_idx < NUM_BUFFERS)
                else $error("[%0t] SVA Chyba: Ukazovateľ find_full_start_idx (%0d) prekročil rozsah!", $time, find_full_start_idx);
        end
    end
    `endif
