// sdram_controller_final.sv - Verzia 10.2 - FSM Loop Fix
//
// Zmeny (v10.2) - Oprava zaseknutia FSM:
// 1. ROBUSTNOSŤ (FSM): Opravená kritická chyba v stave `EVAL_CMD`, kde sa FSM
//    pri čakaní na časovače (tRP, tRCD, tRAS) vracal do stavu `IDLE`,
//    čo spôsobovalo nekonečnú slučku. Logika bola upravená tak, aby FSM
//    zostal v stave `EVAL_CMD` a počkal, kým sa časovač vynuluje.
//
// Zmeny (v10.1):
// 1. Plná integrácia s `sdram_pkg` a odstránenie lokálnych parametrov.
...
module SdramControllerFinal #(
    // --- Parametre ---
    // Všetky konfiguračné parametre sú teraz centralizované v sdram_pkg.sv
    // Tento modul je plne riadený importovaným balíčkom.
    ...
    output logic [2:0]               debug_rd_fifo_level_o, // Príklad šírky
    output logic [2:0]               debug_wr_fifo_level_o  // Príklad šírky
);

    import sdram_pkg::*;

    // --- Lokálne Parametre ---
    localparam integer FIFO_DEPTH_BITS    = 4; // Interná hĺbka FIFO
    ...
    always_comb begin
        ...
        logic do_read_fifo_write, do_read_fifo_read;
        logic fifo_w_full, fifo_w_empty;
        logic fifo_w_wr_en, fifo_w_rd_en;
        logic do_write_fifo_write, do_write_fifo_read;

        // Defaultné priradenia
        state_next          = state_reg; // Dôležité: FSM zostane v stave, ak nie je definovaný prechod
        ...
        case (state_reg)
            ...
            EVAL_CMD: begin
                // OPRAVA (v10.2): FSM teraz počká v stave EVAL_CMD, kým sa časovače
                // nevynulujú, namiesto toho, aby sa vracal do IDLE.
                if (bank_state[cmd_addr.bank] == BANK_IDLE) begin
                    ...
                    if (active_row[cmd_addr.bank] == cmd_addr.row) begin
                         if (trcd_timer == 0) state_next = (current_cmd.rw == READ_CMD) ? READ_CMD : WRITE_CMD;
    ...
    logic resp_valid_comb;
    logic resp_last_comb;
    logic [DATA_WIDTH-1:0] resp_data_comb;
    logic wdata_ready_comb;

    assign resp_valid_comb = !fifo_r_empty;
    assign resp_last_comb  = read_fifo_last[fifo_r_rptr];
    assign resp_data_comb  = read_fifo_data[fifo_r_rptr];
    assign wdata_ready_comb= !fifo_w_full;

    assign resp_valid = resp_valid_comb;
    assign resp_last = resp_last_comb;
    assign resp_data = resp_data_comb;
    assign wdata_ready = wdata_ready_comb;
    ...
    if (ENABLE_DEBUG) begin : g_debug_outputs
        ...
        assign debug_rd_fifo_level_o = fifo_r_count[2:0];
        assign debug_wr_fifo_level_o = fifo_w_count[2:0];
