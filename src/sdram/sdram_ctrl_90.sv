// sdram_controller_final.sv - Verzia 9.0 - Plne integrovaná produkčná verzia
//
// Zmeny (v9.0) - Centralizácia konfigurácie:
// 1. PREVZATIE KONFIGURÁCIE Z BALÍČKA: Modul už nedefinuje vlastné šírky adries
//    (ROW/COL/BANK), ale preberá ich priamo z `sdram_pkg`. Tým sa zjednodušuje
//    inštanciácia a zaručuje konzistencia v celom systéme.
// 2. ODSTRÁNENIE LOKÁLNYCH TYPOV: Miestna definícia `sdram_addr_t` bola odstránená,
//    používa sa verzia z balíčka.
// 3. SILNÉ TYPY: Logika FSM teraz používa enum `rw_cmd_t` pre porovnávanie
//    príkazov, čo zvyšuje čitateľnosť a bezpečnosť.
//
// Zmeny (v8.1):
// 1. INTEGRÁCIA BALÍČKA: Modul používa dátový typ `sdram_cmd_t` z `sdram_pkg.sv`.
//
// Zmeny (v8.0):
// 1. ADRESOVÁ ŠTRUKTÚRA, POMENOVANÉ KONŠTANTY, PRIORITNÝ REFRESH, VALIDÁCIA PARAMETROV.
...
module SdramControllerFinal #(
    // --- Parametre ---
    // Konfigurácia adresy sa teraz preberá z `sdram_pkg`
    ...
    // Import definícií z externého balíčka
    import sdram_pkg::*;

    // --- Lokálne Parametre (odvodené z `sdram_pkg` a parametrov modulu) ---
