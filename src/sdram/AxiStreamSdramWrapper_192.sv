// AxiStreamSdramWrapper.sv - Verzia 1.92 - Quartus always_ff Compatibility Fix
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v1.92):
// 1. KOMPATIBILITA (Quartus): Deklarácie lokálnych premenných v `always_ff` bloku
//    boli presunuté na jeho začiatok, aby sa predišlo syntaktickým chybám.
//
// Zmeny (v1.91):
// 1. Nahradenie operátora `inside` pre lepšiu kompatibilitu so staršími verziami Quartus.
//
// Zmeny (v1.90):
// 1. Pridaný externý vstup `soft_reset_i`.
// 2. Pridané nové ladiace výstupy na detailné monitorovanie.
...
module AxiStreamSdramWrapper #(
    ...
    always_ff @(posedge clk) begin
        // REFAKTORING (v1.92): Deklarácie presunuté na začiatok bloku
        logic word_accepted;
        logic close_write_segment;
        logic is_last_write_burst;
        logic last_word_of_read_segment;
        ...
        if (!rstn || internal_reset || soft_reset_i) begin
            ...
            word_accepted = s_axis_tready && s_axis_tvalid;
            ...
            close_write_segment = word_accepted && (s_axis_tlast || (write_segment_word_count == SEGMENT_LEN_WORDS - 1));
            ...
            is_last_write_burst = (issue_write_cmd && cmd_fifo_ready &&
            ...
            last_word_of_read_segment = m_axis_tvalid && m_axis_tready && (read_segment_word_count == segment_actual_len_words[read_ptr] - 1);
            ...
            // REFAKTORING (v1.91): Nahradenie `inside` pre lepšiu kompatibilitu
