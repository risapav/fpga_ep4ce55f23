// AxiStreamSdramWrapper.sv - Verzia 1.99 - Final Refactoring with Functions
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v1.99):
// 1. ROBUSTNOSŤ (Functions): Procedurálne `for` cykly pre výpočet debug
//    počítadiel boli zapuzdrené do dedikovaných funkcií. Tento prístup je
//    čistejší, modulárnejší a poskytuje syntetizátoru jednoznačný popis
//    pre optimálnu implementáciu kombinačnej logiky.
//
// Zmeny (v1.98):
// 1. Pevne zakódované sčítanie bolo nahradené za generický `for` cyklus.
...
module AxiStreamSdramWrapper #(
    ...
    // REFAKTORING (v1.99): Funkcie pre výpočet aktívnych bufferov
    function automatic logic [$clog2(NUM_BUFFERS+1)-1:0] count_active_reads (
        logic states [0:NUM_BUFFERS-1]
    );
        logic [$clog2(NUM_BUFFERS+1)-1:0] count = '0;
        for (int k = 0; k < NUM_BUFFERS; k = k + 1) begin
            count = count + states[k];
        end
        return count;
    endfunction

    function automatic logic [$clog2(NUM_BUFFERS+1)-1:0] count_active_writes (
        buffer_state_t states [0:NUM_BUFFERS-1]
    );
        logic [$clog2(NUM_BUFFERS+1)-1:0] count = '0;
        for (int k = 0; k < NUM_BUFFERS; k = k + 1) begin
            if (states[k] == FILLING || states[k] == FULL) begin
                count = count + 1;
            end
        end
        return count;
    ...
    always_comb begin
        ...
        // --- Debug výstupy ---
        // REFAKTORING (v1.99): Použitie funkcií pre čistotu a robustnosť
        dbg_active_reads_comb = count_active_reads(read_cmd_issued);
        dbg_active_writes_comb = count_active_writes(buffer_state);
