// AxiStreamSdramWrapper.sv - Verzia 2.01 - Robust Pointer Arithmetic
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v2.01):
// 1. ROBUSTNOSŤ (Circular Buffer): Aritmetika pre inkrementáciu ukazovateľov
//    (write_ptr, read_ptr, lookahead_ptr) bola upravená tak, aby používala
//    modulo operátor. Tým je zaručená správna "wrap-around" funkčnosť
//    kruhového buffera pre akýkoľvek počet bufferov (NUM_BUFFERS),
//    nielen pre mocniny čísla 2.
//
// Zmeny (v2.00):
// 1. Funkcie pre výpočet debug počítadiel boli odstránené a ich logika
//    bola vrátená priamo do `always_comb` bloku pre maximalizáciu kompatibility.
    ...
    always_ff @(posedge clk) begin
        ...
        if (!rstn || internal_reset || soft_reset_i) begin
            ...
            if (buffer_state[write_ptr] == FULL && is_last_write_burst_comb) begin
                // REFAKTORING (v2.01): Zabezpečuje správne "pretočenie" ukazovateľa pre akýkoľvek NUM_BUFFERS
            ...
            if (buffer_state[read_ptr] == READING && last_word_of_read_segment_comb) begin
                ...
                // REFAKTORING (v2.01): Zabezpečuje správne "pretočenie" ukazovateľa
