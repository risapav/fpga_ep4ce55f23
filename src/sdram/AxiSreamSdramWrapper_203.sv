// AxiStreamSdramWrapper.sv - Verzia 2.03 - Final Synthesis Fix
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v2.03):
// 1. ROBUSTNOSŤ (Synthesis): Viacnásobné priradenia na jednom riadku v
//    resetovacej logike boli rozdelené na samostatné riadky. Tento prístup
//    rieši problémy s parserom v nástroji Quartus a zvyšuje čitateľnosť.
//
// Zmeny (v2.02):
// 1. Procedurálny `for` cyklus v resetovacej logike bol nahradený za
//    robustné agregačné priradenie (`'{default:...}`).
...
module AxiStreamSdramWrapper #(
    ...
    always_ff @(posedge clk) begin
        ...
        if (!rstn || internal_reset || soft_reset_i) begin
            ...
            // REFAKTORING (v2.03): Rozdelenie priradení na samostatné riadky
            // pre maximálnu kompatibilitu so syntetizátormi.
            write_ptr <= '0;
            read_ptr <= '0;
            ...
            w_pipeline_rptr <= '0;
            w_pipeline_level <= '0;
            write_frame_finished <= 1'b0;
            last_write_ptr_for_frame <= '0;
            timeout_active <= 1'b0;
            timeout_counter <= '0;
            dbg_active_reads <= '0;
            dbg_active_writes <= '0;
