// AxiStreamSdramWrapper.sv - Verzia 1.98 - Generic Debug Counters Fix
// Modul, ktorý slúži ako most medzi AXI-Stream rozhraním a SDRAM kontrolérom.
// Zmeny (v1.98):
// 1. ROBUSTNOSŤ (Generic Counters): Pevne zakódované sčítanie pre debug
//    počítadlá bolo nahradené za generický `for` cyklus, aby sa predišlo
//    chybám pri prístupe mimo rozsah poľa, ak sa zmení parameter `NUM_BUFFERS`.
//
// Zmeny (v1.97):
// 1. Nepodporovaná systémová funkcia `$countones` bola nahradená za sčítanie.
...
module AxiStreamSdramWrapper #(
    ...
    // REFAKTORING (v1.98): Kombinačné signály pre debug počítadlá
    ...
    always_ff @(posedge clk) begin
        if (!rstn || internal_reset || soft_reset_i) begin
            ...
            // --- Debug counters ---
            // REFAKTORING (v1.98): Hodnoty sa teraz počítajú v kombinačnom bloku a tu sa len registrujú.
    ...
    always_comb begin
        ...
        // --- Debug výstupy ---
        // REFAKTORING (v1.98): Generický výpočet počtu aktívnych bufferov
        dbg_active_reads_comb = '0;
        dbg_active_writes_comb = '0;
        for (int j = 0; j < NUM_BUFFERS; j = j + 1) begin
            dbg_active_reads_comb  = dbg_active_reads_comb + read_cmd_issued[j];
            dbg_active_writes_comb = dbg_active_writes_comb + (buffer_state[j] == FILLING || buffer_state[j] == FULL);
        end
